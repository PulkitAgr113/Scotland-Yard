			...
			while(true){
				...
				*/
				...
				if (cmd == null){
					...
					client_quit = true;
					if(this.id != -1 || !fugitiveErased) {
						this.board.threadInfoProtector.acquire();
						this.board.erasePlayer(this.id);
						if(this.id == -1) fugitiveErased = true;
						this.board.threadInfoProtector.release();
					}
				...
				*/
				this.board.threadInfoProtector.acquire();
				if(!quit) {
					if(this.id == -1) this.board.moveFugitive(target);
				...
				*/
				...
				this.board.count ++;
				int board_cnt = this.board.count;
				this.board.threadInfoProtector.acquire();
				int playThreads = this.board.playingThreads;
				this.board.threadInfoProtector.release();
				this.board.countProtector.release();
				if(board_cnt == playThreads) {
					this.board.barrier1.release(board_cnt);
				}
				...
				*/
				...
				if (!client_quit){
					...
					if(this.id == -1) feedback = this.board.showFugitive();
				...
				*/
				this.board.countProtector.acquire();
				this.board.count--;
				int board_cnt_2 = this.board.count;
				this.board.threadInfoProtector.acquire();
				int thread_cnt = this.board.playingThreads;
				this.board.threadInfoProtector.release();
				this.board.countProtector.release();

				if(board_cnt_2 == 0) {
					this.board.barrier2.release(thread_cnt);
				}
